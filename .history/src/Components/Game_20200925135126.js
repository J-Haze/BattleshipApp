...
function Game() {
    ...
    function autoPlace(user) {
        if (user == "player") {
            while (!playerState.gameboard.boatsPlaced.includes("carrier")) {
                randomID = getRandomInt(0, 99)
                console.log("randomID:", randomID);
                orientationNumber = getRandomInt(0, 1)
                console.log("orientationNumber:", orientationNumber);
                (orientationNumber == 0) ? randomOrientation = "horizontal" : randomOrientation = "vertical";
                console.log("rand orient:", randomOrientation);
                playerState.gameboard.placeShip("carrier", randomID, randomOrientation);
            }
            while (!playerState.gameboard.boatsPlaced.includes("battleship")) {
                randomID = getRandomInt(0, 99)
                console.log("randomID:", randomID);
                orientationNumber = getRandomInt(0, 1)
                console.log("orientationNumber:", orientationNumber);
                (orientationNumber == 0) ? randomOrientation = "horizontal" : randomOrientation = "vertical";
                console.log("rand orient:", randomOrientation);
                playerState.gameboard.placeShip("battleship", randomID, randomOrientation);
            }
            setPlayer(playerState)
            setUpdate(!update)
        }
